...
The file tested_output.txt has been included to show the execution of the script and to compare to the screenshots from the Expasy Translate tool (Swiss Institute of Bioinformatics, n.d.). Screenshots of the first two translated sequences by Expasy have been included and are named Expasy_seq1.png and Expasy_seq2.png. The results between the two are almost completely consistent, with the following exceptions:
...
2. When two start codons are present before a stop codon is encountered, the Expasy Translate tool refers to this as a single ORF. This script considers that due to mRNA folding, ribosome specificity, or other factors, this may confer in two separate protein isoforms of the gene (Bazykin, 2011). As such, both are listed in the output for that specific frame. Protein isoforms, generated by mechanisms like the use of multiple start codons and alternative splicing, can have vastly different functions and interactions and are therefore important prospects for future studies that investigate the complexity of the transcriptome. 

While we touched on Biopython methods from the Bio.Seq module in week 2, I decided to approach this assignment using basic python loops to reinforce my knowledge before moving on to the premade methods. This script is a great introductory in silico DNA to protein translator, there are some caveats, primarily being that the length of the ORF is not being considered. There are multiple examples in the tested_output.txt file that result in a single amino acid protein product - Methionine. In reality this, and other sequences of only a few amino acids in length, would not be an appropriate marker of a true ORF (DNASTAR, n.d.). 
...
DNASTAR. (n.d.). Coding prediction – starts stops ORFs. GeneQuest User Guide. Retrieved January 27, 2025, from https://www.dnastar.com/manuals/genequest/17.3/en/topic/coding-prediction-starts-stops-orfs

Swiss Institute of Bioinformatics. (n.d.). Expasy translate tool. Expasy. Retrieved January 27, 2025, from https://web.expasy.org/translate/

Zhan, X. (2023). Codon table [Data set]. GitHub. https://github.com/zhanxw/anno/blob/master/codon.txt
